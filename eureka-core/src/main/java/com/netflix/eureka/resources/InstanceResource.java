...
public class InstanceResource {
  ...
  /**
   * Removes status override for an instance, set with {@link #statusUpdate(String, String,
   * String)}.
   *
   * @param isReplication a header parameter containing information whether this is replicated from
   *     other nodes.
  ...
  public Response deleteStatusUpdate(
    ...
    try {
      ...
      InstanceStatus newStatus =
          newStatusValue == null ? InstanceStatus.UNKNOWN : InstanceStatus.valueOf(newStatusValue);
      boolean isSuccess =
          registry.deleteStatusOverride(
              app.getName(), id, newStatus, lastDirtyTimestamp, "true".equals(isReplication));
   ...
   *
   * @return response containing information about the the instance's {@link InstanceInfo}.
   */
  @GET
  ...
  public Response renewLease(
    ...
    // 检查我们是否需要根据脏时间戳进行同步，客户端实例可能已经更改了一些值
    Response response;
    ...
    logger.debug(
        "Found (Renew): {} - {}; reply status={}", app.getName(), id, response.getStatus());
    return response;
  }
   ...
   *
   * <p>The status updates are normally done for administrative purposes to change the instance
   * status between {@link InstanceStatus#UP} and {@link InstanceStatus#OUT_OF_SERVICE} to select or
   * remove instances for receiving traffic.
   *
  ...
  /**
   * Updates user-specific metadata information. If the key is already available, its value will be
   * overwritten. If not, it will be added.
   *
  ...
  public Response updateMetadata(@Context UriInfo uriInfo) {
    try {
      ...
      if (instanceInfo == null) {
        logger.warn(
            "Cannot find instance while updating metadata for instance {}/{}", app.getName(), id);
        return Response.status(Status.NOT_FOUND).build();
      }
