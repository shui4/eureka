 ...
 *
 * <p>Primary operations that are performed are the <em>Registers</em>, <em>Renewals</em>,
 * <em>Cancels</em>, <em>Expirations</em>, and <em>Status Changes</em>. The registry also stores
 * only the delta operations
 *
...
public abstract class AbstractInstanceRegistry implements InstanceRegistry {
  ...
  /**
   * 为什么要多此一举还要弄个这个？首先多次注册，可以先通过它来记录一下最新的修改状态 参考 {@link
   * AbstractInstanceRegistry#statusUpdate(String, String, InstanceStatus, String, boolean)}， 然后它还配合
   * {@link OverrideExistsRule} 做一些事情即续约和注册的时候 计算client真正的状态
   */
  ...
  protected AbstractInstanceRegistry(
    ...
    // 开启对“最近变更队列recentlyChangedQueue” 中元素的定时清除任务
  ...
  private TimerTask getDeltaRetentionTask() {
    return new TimerTask() {
      ...
      public void run() {
        ...
        // serverConfig.getRetentionTimeInMSInDeltaQueue() recentlyChangedQueue队列中的recent（最近）时间
        while (it.hasNext()) {
          // 将if()中的条件变化一个形式：
          //          <=>it.next().getLastUpdateTime() < System.currentTimeMillis() -
          // serverConfig.getRetentionTimeInMSInDeltaQueue()
          //          <=> System.currentTimeMillis() -
          // serverConfig.getRetentionTimeInMSInDeltaQueue() > it.next().getLastUpdateTime()
          //          <=> System.currentTimeMillis() - it.next().getLastUpdateTime() <
          // System.currentTimeMillis()
          if (it.next().getLastUpdateTime()
              < System.currentTimeMillis() - serverConfig.getRetentionTimeInMSInDeltaQueue()) {
            it.remove();
          } else {
            // 直接结束？因为recentlyChangedQueue是 链表的方式存储，后面的修改排到后面，所以有一个不符合if就结束了
            break;
          }
        }
      }
  ...
  /**
   * Returns the given app that is in this instance only, falling back to other regions
  ...
  /**
   * Get all applications in this instance registry, falling back to other regions if allowed in the
   * Eureka config.
   *
  ...
  public Applications getApplications() {
    ...
    if (disableTransparentFallback) {
      ...
      return getApplicationsFromAllRemoteRegions(); // Behavior of falling back to remote region can
      // be disabled.
    }
  }
   ...
   *     <p>Get all instances by ID, including automatically asking other regions if the ID is
   *     unknown.
   */
  @Deprecated
   ...
   *     indicated by the region {@link URL} by this property {@link
   ...
   *     <p>Get the list of instances by its unique id.
   */
  @Deprecated
  ...
  /**
   * Get the registry information about the delta changes. The deltas are cached for a window
   * specified by {@link EurekaServerConfig#getRetentionTimeInMSInDeltaQueue()}. Subsequent requests
   * for delta information may return the same information and client must make sure this does not
   * adversely affect them.
   *
   * @return all application deltas.
   * @deprecated use {@link #getApplicationDeltasFromMultipleRegions(String[])} instead. This method
   *     has a flawed behavior of transparently falling back to a remote region if no instances for
   *     an app is available locally. The new behavior is to explicitly specify if you need a remote
   *     region.
   */
  @Deprecated
  public Applications getApplicationDeltas() {
    ...
    try {
      ...
      logger.debug(
          "The number of elements in the delta queue is : {}", this.recentlyChangedQueue.size());
      while (iter.hasNext()) {
   ...
   *     EurekaServerConfig#getRemoteRegionUrls()}, false otherwise
   * @return applications
   * @deprecated Use {@link #getApplicationsFromMultipleRegions(String[])} instead. This method has
   *     a flawed behavior of transparently falling back to a remote region if no instances for an
   *     app is available locally. The new behavior is to explicitly specify if you need a remote
   *     region.
   */
  @Deprecated
  ...
  /**
   * Gets the application delta also including instances from the passed remote regions, with the
   * instances from the local region. <br>
   *
   * <p>The remote regions from where the instances will be chosen can further be restricted if this
   * application does not appear in the whitelist specified for the region as returned by {@link
   * EurekaServerConfig#getRemoteRegionAppWhitelist(String)} for a region. In case, there is no
   * whitelist defined for a region, this method will also look for a global whitelist by passing
   * <code>null</code> to the method {@link EurekaServerConfig#getRemoteRegionAppWhitelist(String)}
   * <br>
   *
   * @param remoteRegions The remote regions for which the instances are to be queried. The
   *     instances may be limited by a whitelist as explained above. If <code>null</code> all remote
   *     regions are included. If empty list then no remote region is included.
   * @return The delta with instances from the passed remote regions as well as local region. The
  ...
  public Applications getApplicationDeltasFromMultipleRegions(String[] remoteRegions) {
    ...
    try {
      ...
      logger.debug(
          "The number of elements in the delta queue is :{}", this.recentlyChangedQueue.size());
      while (iter.hasNext()) {
        // 根据元素队列，获取到其包含lease及instanceInfo
        Lease<InstanceInfo> lease = iter.next().getLeaseInfo();
        InstanceInfo instanceInfo = lease.getHolder();
        logger.debug(
            "The instance id {} is found with status {} and actiontype {}",
            ...
            instanceInfo.getActionType().name());
        Application app = applicationInstancesMap.get(instanceInfo.getAppName());
        // 将app添加到apps中，再将lease封装为instanceInfo写入到app，然后写入到apps
        if (app == null) {
          app = new Application(instanceInfo.getAppName());
          applicationInstancesMap.put(instanceInfo.getAppName(), app);
          apps.addApplication(app);
        }
        // 1）为什么decorateInstanceInfo()返回的就是InstanceInfo，这里还需要再将其创建为一个InstanceInfo？
        //    decorateInstanceInfo（lease）返回的instanceInfo实例，实际是来自于recentlyChangedQueue的实例。
        // 若在当前方法结束后，在当前方法的返回值apps还未发送给client之前，某个lease的的数据又被修改（write锁解开之后，
        // 别的线程就可以从recentlyChangedQueue获取到同样的app实例，这种情况是不可以的），
        //    那么发送给client的这个数据就是最新的二次修改的数据。这种情况下回导致client接收到这个增量数据中丢失了
        // 一次修改时的情况，而这个丢失会引发client进行全量下载。为了避免这种情况的发送，使用decorateInstanceInfo()返回的instanceInfo数据再创建一个新的instanceInfo，
        // 使当前方法返回的这个apps中的instanceInfo与recentlyChangedQueue没有关系。

        // 2）为什么全量下载时没有做这里的第二次创建操作？
        // 全量下载使用的是注册表，与recentlyChangedQueue无关。
        // 若在当前方法（全量下载方法）结束后，在当前的返回值apps还未发送给client之前，
        // 某个lease的数据又被修改了。那么发送给client的这个数据就是最新的二次修改的数据
        // 二次修改前的数据对于client来说就丢失了。对于客户端来说，这次丢失是没问题的，所以不需要将decorateInstanceInfo()的结果再次创建新的对象。
        app.addInstance(new InstanceInfo(decorateInstanceInfo(lease)));
      }
  ...
  /**
   * This method will return applications with instances from all passed remote regions as well as
   * the current region. Thus, this gives a union view of instances from multiple regions. <br>
   * The application instances for which this union will be done can be restricted to the names
   * returned by {@link EurekaServerConfig#getRemoteRegionAppWhitelist(String)} for every region. In
   * case, there is no whitelist defined for a region, this method will also look for a global
   * whitelist by passing <code>null</code> to the method {@link
   * EurekaServerConfig#getRemoteRegionAppWhitelist(String)} <br>
   * If you are not selectively requesting for a remote region, use {@link
   * #getApplicationsFromAllRemoteRegions()} or {@link #getApplicationsFromLocalRegionOnly()}
   *
   * @param remoteRegions The remote regions for which the instances are to be queried. The
   *     instances may be limited by a whitelist as explained above. If <code>null</code> or empty
   *     no remote regions are included.
   * @return The applications with instances from the passed remote regions as well as local region.
  ...
  public Applications getApplicationsFromMultipleRegions(String[] remoteRegions) {
    ...
    // 获取远程region下的Application添加到Applications
    if (includeRemoteRegion) {
      for (String remoteRegion : remoteRegions) {
        RemoteRegionRegistry remoteRegistry = regionNameVSRemoteRegistry.get(remoteRegion);
        if (null != remoteRegistry) {
   ...
   * Same as calling {@link #getApplicationsFromMultipleRegions(String[])} with a <code>null</code>
   * argument.
   */
  ...
  @com.netflix.servo.annotations.Monitor(
      name = "numOfElementsinInstanceCache",
  ...
  public void register(InstanceInfo registrant, int leaseDuration, boolean isReplication) {
    ...
    try {
      ...
      if (existingLease != null && (existingLease.getHolder() != null)) {
        ...
        logger.debug(
            "Existing lease found (existing={}, provided={}",
            existingLastDirtyTimestamp,
            registrationLastDirtyTimestamp);

        // this is a > instead of a >= because if the timestamps are equal, we still take the remote
        // transmitted
      ...
      // 初始注册的时候 给过来的instance 状态一定是 UNKNOWN，指第一次注册不成立的意思，只有在 后面修改状态，定时更新客户端应用程序本地的状态
      // 然后修改配置文件发起的注册、或者人工调用的时候，状态才不会是 UNKNOWN
      if (!InstanceStatus.UNKNOWN.equals(registrant.getOverriddenStatus())) {
        logger.debug(
            ...
            registrant.getId());
        // 人工直接提交修改请求，将状态改成XX比如UP，然后客户端应用程序再因为注册文件发送注册请求  改成 xx状态 是不允许的
        // 为什么不允许？因为 客户端应用程序配置文件发生变化的时候是会发送注册请求数据可能是old的，所以它选择相信 人工请求的状态是最新的
      ...
      invalidateCache(
          registrant.getAppName(), registrant.getVIPAddress(), registrant.getSecureVipAddress());
      logger.info(
          "Registered instance {}/{} with status {} (replication={})",
          registrant.getAppName(),
          registrant.getId(),
          registrant.getStatus(),
          isReplication);
    } finally {
      read.unlock();
    }
   ...
   *
   * <p>This is normally invoked by a client when it shuts down informing the server to remove the
   * instance from traffic.
   *
   * @param appName the application name of the application.
   * @param id the unique identifier of the instance.
   * @param isReplication true if this is a replication event from other nodes, false otherwise.
   * @return true if the instance was removed from the {@link AbstractInstanceRegistry}
  ...
  /**
   * {@link #cancel(String, String, boolean)} method is overridden by {@link
   * PeerAwareInstanceRegistry}, so each cancel request is replicated to the peers. This is however
   * not desired for expires which would be counted in the remote peers as valid cancellations, so
   * self preservation mode would not kick-in.
   */
  protected boolean internalCancel(String appName, String id, boolean isReplication) {
    ...
    try {
      ...
      if (leaseToCancel == null) {
        ...
        logger.warn(
            "DS: Registry: cancel failed because Lease is not registered for: {}/{}", appName, id);
        return false;
      } else {
  ...
  /**
   * Marks the given instance of the given app name as renewed, and also marks whether it originated
   * from replication.
   *
  ...
  public void evict(long additionalLeaseMs) {
    ...
    // 我们首先收集所有过期的物品，以随机顺序驱逐它们。
    // 对于大型驱逐集，如果我们不这样做，我们可能会在自我保护开始之前清除整个应用程序。
    ...
    if (toEvict > 0) {
      logger.info(
          "Evicting {} items (expired={}, evictionLimit={})",
          toEvict,
          expiredLeases.size(),
          evictionLimit);
      ...
      for (int i = 0; i < toEvict; i++) {
        // 洗牌算法
        // 选择一个随机目标（Knuth shuffle 算法）
        int next = i + random.nextInt(expiredLeases.size() - i);
        // 将expiredLeases列表中第i与第next个元素互换
  ...
  protected void updateRenewsPerMinThreshold() {
    /*
        每分钟更新次数阈值 = (客户端数量* (60/心跳间隔)* 自我保护开启的闽值因子)
   ...
   *     <p>Stores overridden status if it is not already there. This happens during a
   *     reconciliation process during renewal requests.
   */
  @Deprecated
  @Override
  public void storeOverriddenStatusIfRequired(String id, InstanceStatus overriddenStatus) {
    ...
    if ((instanceStatus == null) || (!overriddenStatus.equals(instanceStatus))) {
      // We might not have the overridden status if the server got restarted -this will help us
      // maintain
  ...
  /**
   * Updates the status of an instance. Normally happens to put an instance between {@link
   * InstanceStatus#OUT_OF_SERVICE} and {@link InstanceStatus#UP} to put the instance in and out of
   * traffic.
   *
   * @param appName the application name of the instance.
   * @param id the unique identifier of the instance.
   ...
   *
   * @return the integer representing the threshold for the renewals per minute.
   */
  @com.netflix.servo.annotations.Monitor(
      name = "numOfRenewsPerMinThreshold",
      type = DataSourceType.GAUGE)
  @Override
  ...
  protected void initRemoteRegionRegistry() throws MalformedURLException {
    ...
    if (!remoteRegionUrlsWithName.isEmpty()) {
      ...
      for (Map.Entry<String, String> remoteRegionUrlWithName :
          remoteRegionUrlsWithName.entrySet()) {
  ...
  /* visible for testing */ class EvictionTask extends TimerTask {
    ...
    /**
     * compute a compensation time defined as the actual time this task was executed since the prev
     * iteration, vs the configured amount of time for execution. This is useful for cases where
     * changes in time (due to clock skew or gc for example) causes the actual eviction task to
     * execute later than the desired time according to the configured cycle.
     */
    long getCompensationTimeMs() {
      ...
      // 假设每次任务需要6s，我们给它设置的间隔(getEvictionIntervalTimerInMs)是5s
      // 执行2次
      // 1)[0-5]：5+6=11s结束
      // 2)[5-10]:达到5s不执行，因为1）还没结束。到11s 1)调度结束 时：11-5（上一次开始）-5(getEvictionIntervalTimerInMs)=1（补偿时间）
